...
/**
 * An asynchronous task that executes on a background thread pool.
 *
 * <p>Async tasks are ideal for operations that don't need to access
 * Minecraft world state, such as:
 * <ul>
 *   <li>Database queries and updates</li>
 *   <li>File I/O operations</li>
 *   <li>HTTP requests to external services</li>
 *   <li>Heavy computations that would block the main thread</li>
 * </ul>
 *
 * <h2>Important</h2>
 * <p>Async tasks must never directly access Minecraft APIs that are not
 * thread-safe. Use {@link sh.pcx.unified.scheduler.SchedulerService#runTask}
 * to switch back to the main thread for world modifications.
 *
 * <h2>Example Usage</h2>
 * <pre>{@code
 ...
 * AsyncTask autoSave = new AsyncTask(
...
public class AsyncTask implements Task {
    ...
    /**
     * Creates a new asynchronous task with a runnable.
     *
     * @param runnable    the task to execute
     * @param delayTicks  initial delay in ticks
     * @param periodTicks period between executions (0 for one-time)
     */
    public AsyncTask(@NotNull Runnable runnable, long delayTicks, long periodTicks) {
        this(null, runnable, null, delayTicks, periodTicks, null, null, null);
    }

    /**
     * Creates a new asynchronous task with full configuration.
     *
